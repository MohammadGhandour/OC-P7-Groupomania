<template>
    <Navbar :profileImage="connectedUserProfile" :username="connectedUserUsername" />
    <div class="container">
        <div class="profile-infos">
            ...
            <div class="profile-infos-content">
                ...
                <div class="block credentials">
                    ...
                    <router-link to="/edit" class="edit" v-if="connectedUserId == profileUserId">Modifier</router-link>
                </div>
            </div>
        </div>
        ...
        <div class="post" v-for="post in userPosts" :key="post" :data-id="post.id">
            <div class="post-header">
                ...
                <i class="fa-solid fa-ellipsis" @click="showPostActions" v-if="connectedUserId == profileUserId || connectedUserStatus == 1" title="Actions du post"></i>
                <div class="delete-edit-post hidden">
                    ...
                    <div class="edit-post-btn post-action" @click="showEditPost" v-if="post.userId == connectedUserId">
            ...
            <div class="comment-section">
                <form class="new-comment" @submit.prevent="createComment">
                    ...
                    <input type="text" name="comment" id="comment" placeholder="Ajouter un commentaire public !" v-model="newComment" required>
...
export default {
    ...
    mounted() {
        ...
        Axios.get(urlApi + '/user/' + JSON.parse(localStorage.getItem('userId')), {
            headers: {
                "authorization": 'Bearer ' + JSON.parse(token),
            }
        })
        .then((res) => {
            this.connectedUserUsername = res.data.username;
            this.connectedUserProfile = res.data.profileImage;
        })
    ...
    methods: {
        ...
        handleLike(e) {
            ...
            .then((res) => {
                if(res) {
                    this.getUserPosts(profileUserId, token);
                }
            })
            .catch((err) => {
                console.log(err);
        ...
        getUserPosts(profileUserId, token) {
            Axios.get(urlApi + `/posts/${profileUserId}`, {
                headers: {
                    "authorization": 'Bearer ' + JSON.parse(token),
                }
            })
...
<style scoped src="../../public/styles/user-profile.css"></style>